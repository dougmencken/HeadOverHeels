 ...
 * */
static int save_indexed(png_structp png_ptr, BITMAP *picture)
{
    ASSERT(bitmap_color_depth(picture) == 8);

    if (is_memory_bitmap(picture)) { /* fast path */
        int y;

        for (y=0; y<picture->h; y++) {
            png_write_row(png_ptr, picture->line[y]);
        }
        ...
        rowdata = (unsigned char *)malloc(picture->w * 3);
        if (!rowdata)
            return 0;
 ...
 */
static int save_rgb(png_structp png_ptr, BITMAP *picture)
{
    AL_CONST int depth = bitmap_color_depth(picture);
    unsigned char *rowdata;
    int y, x;
 ...
 */
static int save_rgba(png_structp png_ptr, BITMAP *picture)
{
    unsigned char *rowdata;
    int x, y;

    ASSERT(bitmap_color_depth(picture) == 32);

    rowdata = (unsigned char *)malloc(picture->w * 4);
    if (!rowdata)
        return 0;
...
static int really_save_png(PACKFILE *fp, BITMAP *picture, AL_CONST RGB *pal)
{
    ...
    depth = bitmap_color_depth(picture);
    if (depth == 8 && !pal)
        return -1;
    ...
    switch (depth) {
        ...
        case 16:
        case 24:
            if (!save_rgb(png_ptr, picture))
                goto Error;
            break;
        case 32:
            if (!save_rgba(png_ptr, picture))
                goto Error;
            break;
        default:
            ASSERT(FALSE);
...
int save_png(AL_CONST char *filename, BITMAP *picture, AL_CONST RGB *pal)
{
    PACKFILE *fp;
    int result;
    ...
    result = really_save_png(fp, picture, pal);
