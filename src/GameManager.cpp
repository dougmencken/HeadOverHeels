...
void GameManager::refreshSceneryBackgrounds ()
{
        ...
        sceneryBackgrounds[ "themoon" ] = refreshPicture( "themoon-frame.png" );
...
void GameManager::drawAmbianceOfGame ( const allegro::Pict& where )
{
        ...
        if ( ! hasBackgroundPicture () )
        {
                ...
                if ( isomot.getMapManager().getActiveRoom() != nilPointer )
                {
                        ...
                        allegro::textOut( xCamera, where, textX, textY, Color::whiteColor().toAllegroColor() );
                        allegro::textOut( "," , where, textX + widthOfChar * xCamera.length(), textY, Color::gray50Color().toAllegroColor() );
                        allegro::textOut( yCamera, where, textX + widthOfChar * ( xCamera.length() + 1 ), textY, Color::whiteColor().toAllegroColor() );
                }
        }
}
