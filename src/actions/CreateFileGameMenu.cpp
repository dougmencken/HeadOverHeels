...
using gui::CreateFileGameMenu;

CreateFileGameMenu::CreateFileGameMenu(BITMAP* destination, bool isLoadMenu)
: Action(),
...
void CreateFileGameMenu::execute()
{
  Label* label = 0;
  ...
  for(int fileCount = 1; fileCount < 10; fileCount++)
  {
    ...
    if(exists(file.c_str()))
    {
      ...
      ss.str(std::string());
      ss << fileCount << ". " << rooms << " " << languageManager->findLanguageString("rooms")->getText() << "; " << planets << " " << languageManager->findLanguageString("planets")->getText();
