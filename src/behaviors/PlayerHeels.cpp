...
PlayerHeels::PlayerHeels( Item* item, const BehaviorId& id ) : UserControlled( item, id )
{
      ...
      stateId != StateStartDestroy && stateId != StateDestroy )
  {
    ...
    if( stateId == StateWait || stateId == StateBlink )
    {
      ...
      else if( input->jump() )
      {
        ...
        stateId = ( playerItem->getMediator()->collisionWith( TeleportBehavior ) ? StateStartWayOutTeletransport : StateJump );
      }
    }
    // Si está moviéndose
