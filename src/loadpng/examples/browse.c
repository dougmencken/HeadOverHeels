...
load_and_blit(AL_CONST char *filename)
{
	...
	generate_optimized_palette(pic, pal, rsvd);
    ...
    if (depth == 32) {
	set_alpha_blender();
	draw_trans_sprite(dbuf, pic, 100, 100);
    } else {
#if 0
	blit(pic, dbuf, 0, 0, 0, 0, pic->w, pic->h);
#else
	draw_sprite(dbuf, pic, 100, 100);
#endif
    }
    ...
    textprintf(screen, font, 0, SCREEN_H-8, makecol(255, 255, 255),
	       "%s (%dx%dx%d) gamma=%f", filename, pic->w, pic->h, depth,
	       _png_screen_gamma);
    release_screen();
