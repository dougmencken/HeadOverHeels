...
public:
        ...
        safeptr( ) : pointer( nilPointer )
        {
                ...
                IF_DEBUG( prettyprint( "/* constructor */ safeptr( )", util::toStringWithOrdinalSuffix( countreferences() ) + " reference to " + util::pointer2string( pointer ) ) )
        }
        ...
        explicit safeptr( Type * ptr ) : pointer( ptr )
        {
                ...
                IF_DEBUG( prettyprint( "/* constructor */ safeptr( TYPE * )", util::toStringWithOrdinalSuffix( countreferences() ) + " reference to " + util::pointer2string( pointer ) ) )
        }
        ...
        safeptr( const safeptr < Type > & copy ) : pointer( copy.pointer )
        {
                ...
                IF_DEBUG( prettyprint( "/* copy constructor */ safeptr( const safeptr < TYPE > & )", util::toStringWithOrdinalSuffix( countreferences() ) + " reference to " + util::pointer2string( pointer ) ) )
        }
        ...
        safeptr( const safeptr < ParentType > & copy ) : pointer( copy.getptr() )
        {
                ...
                IF_DEBUG( prettyprint( "/* ctor */ safeptr( const safeptr < " + util::demangle( typeid( ParentType ).name () ) + " > & )", util::toStringWithOrdinalSuffix( countreferences() ) + " reference to " + util::pointer2string( pointer ) ) )
        }
        ...
        void clear ()
        {
                ...
                if ( pointer != nilPointer )
                {
                        ...
                        IF_DEBUG( prettyprint( "clear ()", util::number2string( howmanyrefs ) + " remaining references to " + util::pointer2string( pointer ) ) )

                        if ( howmanyrefs == 0 )
                        {
                                IF_DEBUG( prettyprint( "clear ()", "deleting pointer " + util::pointer2string( pointer ) ) )
                                delete pointer ;
                        }
        ...
        Type & operator * () const
        {
                if ( pointer == nilPointer )
                {
                        IF_DEBUG( prettyprint( "TYPE & operator * () const", "dereferencing NULL" ) )
        ...
        {
                IF_DEBUG( prettyprint( "safeptr < TYPE > & operator = ( const safeptr < TYPE > & that )", "this = that " + that.tostring() ) )
                ...
                addreference();
                IF_DEBUG( prettyprint( "safeptr < TYPE > & operator = ( const safeptr < TYPE > & that )", util::toStringWithOrdinalSuffix( countreferences() ) + " reference to " + util::pointer2string( pointer ) ) )
...
private:
        ...
        operator Type * () const
        {
                IF_DEBUG( prettyprint( "operator TYPE * () const" ) )
                return pointer ;
        }

        safeptr < Type > & operator = ( Type * that )
        {
                IF_DEBUG( prettyprint( "safeptr < TYPE > & operator = ( TYPE * that )", "this = that " + ( that != nilPointer ? util::demangle( typeid( *that ).name () ) + " * " + util::pointer2string( that ) : "0" ) ) )
                ...
                addreference();
                IF_DEBUG( prettyprint( "safeptr < TYPE > & operator = ( TYPE * that )", util::toStringWithOrdinalSuffix( countreferences() ) + " reference to " + util::pointer2string( pointer ) ) )
...
#else
                ( void ) function ; ( void ) message ; ( void ) newline ;
#endif
        }
