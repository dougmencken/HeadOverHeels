  ...
  namespace cxx
  {
    namespace tree
    {
      ...
      stream_extraction_map ()
      {
        ...
        typedef day<C, simple_type> day;
        register_type (
          qualified_name (bits::gday<C> (), xsd),
          &extractor_impl<S, day >,
          false);

        typedef month<C, simple_type> month;
        register_type (
          qualified_name (bits::gmonth<C> (), xsd),
          &extractor_impl<S, month>,
          false);

        typedef month_day<C, simple_type> month_day;
        register_type (
          qualified_name (bits::gmonth_day<C> (), xsd),
          &extractor_impl<S, month_day>,
          false);

        typedef year<C, simple_type> year;
        register_type (
          qualified_name (bits::gyear<C> (), xsd),
          &extractor_impl<S, year>,
          false);

        typedef year_month<C, simple_type> year_month;
        register_type (
          qualified_name (bits::gyear_month<C> (), xsd),
          &extractor_impl<S, year_month>,
          false);
      ...
      extract (istream<S>& s, flags f, type* c)
      {
      ...
      std::auto_ptr<type>
      extractor_impl (istream<S>& s, flags f, type* c)
      {
